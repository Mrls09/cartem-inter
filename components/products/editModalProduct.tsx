...
const TrashBin: React.FC<TrashBinProps> = ({ onDrop }) => {
    ...
    return (
        <div
            ...
            className={`p-4 mt-4 border-2 border-dashed rounded-lg flex items-center justify-center transition-colors ${
                isOver ? 'bg-red-100 border-red-500' : 'border-gray-300'
            }`}
        >
...
const EditModalProductAttachment: React.FC<EditModalProductAttachmentProps> = ({ isOpen, onOpenChange, token, getProduct, product }) => {
    const [subcategories, setSubcategories] = useState<Subcategory[]>([]);
    const [previewImage, setPreviewImage] = useState<string | null>(product?.preview?.url || null);
    ...
    const form = useFormik({
        initialValues: {
            ...
            bulkWholesalePrice: product?.bulkWholesalePrice,
            stock: product?.stock,
            ...
            try {
                ...
                if (response.status === 200) {
                    ...
                    setAlert({
                        type: 'error',
                        title: 'Error',
                        message: errorData.message || 'Ha ocurrido un error al actualizar el producto'
                    });
                }
            } catch (error) {
                setAlert({
    ...
    const renderStep2 = () => (
        <>
            ...
            <ModalFooter>
                <Button color="danger" variant="light" onPress={() => setStep(1)} isDisabled={disableButton}>
                    Atrás
                </Button>
                <Button color="primary" onPress={() => form.handleSubmit()} isDisabled={disableButton}>
    ...
    const closeModal = () => {
        setPreviewImage(product.preview?.url || "");
...
export default EditModalProductAttachment;